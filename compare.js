require("./compare.css");
var $as8nN$vue = require("vue");
var $as8nN$lodash = require("lodash");
var $as8nN$d3shape = require("d3-shape");
var $as8nN$d3scale = require("d3-scale");
var $as8nN$d3array = require("d3-array");
var $as8nN$d3timeformat = require("d3-time-format");
var $as8nN$md5 = require("md5");
require("babel-polyfill");
var $as8nN$localforage = require("localforage");
var $as8nN$jquery = require("jquery");
...
var $c7753e6f10d050d3$export$2e2bcd8739ae039 = ($parcel$interopDefault($as8nN$vue)).extend({
    data () {
        return {
            xAxis: 'launchdate',
            xAxisListener: null,
    ...
    watch: {
        ...
        filenames (filenames) {
            if (filenames.length) {
                const allFuncPaths = ($parcel$interopDefault($as8nN$lodash)).keys(this.runs[0].data);
                // if no rootFuncPath set or dataset doesn't include currently set rootFuncPath set it
                if (this.rootFuncPath == '' || !allFuncPaths.includes(this.rootFuncPath)) this.rootFuncPath = ($parcel$interopDefault($as8nN$lodash)).min(allFuncPaths);
            }
    ...
    computed: {
        ...
        groupedAndAggregated () {
            ...
            peeledMetricData = ($parcel$interopDefault($as8nN$lodash)).map(this.runs, (run)=>{
                var metaPair = ($parcel$interopDefault($as8nN$lodash)).map(run.meta, (meta, metaName)=>[
                        metaName,
                        meta.value
                    ]
                );
                const meta1 = ($parcel$interopDefault($as8nN$lodash)).fromPairs(metaPair);
                var mapPair = ($parcel$interopDefault($as8nN$lodash)).map(run.data, function(metrics, funcPath) {
                ...
                const data = ($parcel$interopDefault($as8nN$lodash)).fromPairs(mapPair);
                return {
                    meta: meta1,
                    data: data
                };
            ...
            const orderedData = ($parcel$interopDefault($as8nN$lodash)).orderBy(peeledMetricData, (item)=>{
                const metaval = item.meta[this.xAxis];
                return parseFloat(metaval) || metaval;
            });
            const grouped = this.selectedGroupBy ? ($parcel$interopDefault($as8nN$lodash)).groupBy(orderedData, (a)=>a.meta[this.selectedGroupBy]
            ) : {
                "all": orderedData
            };
            if (!this.selectedAggregateBy) return grouped;
            const aggregated = ($parcel$interopDefault($as8nN$lodash)).fromPairs(($parcel$interopDefault($as8nN$lodash)).map(grouped, (runList1, groupByName)=>{
                ...
                const aggregateGroups = ($parcel$interopDefault($as8nN$lodash)).groupBy(runList1, (a)=>a.meta[this.xAxis]
                );
                const aggregatedValues = ($parcel$interopDefault($as8nN$lodash)).map(aggregateGroups, (runList, aggregateBykey)=>{
                    const aggregatedRun = {
                        data: ($parcel$interopDefault($as8nN$lodash)).fromPairs(($parcel$interopDefault($as8nN$lodash)).map(runList[0].data, (val, key)=>[
                                key,
                                []
                            ]
                        )),
                        meta: ($parcel$interopDefault($as8nN$lodash)).fromPairs(($parcel$interopDefault($as8nN$lodash)).map(runList[0].meta, (val, key)=>[
                                key,
                                []
                            ]
                        ))
                    ...
                    runList.forEach((run)=>{
                        ($parcel$interopDefault($as8nN$lodash)).forEach(run.meta, (val, key)=>{
                            if (aggregatedRun.meta[key]) aggregatedRun.meta[key].push(val);
                        });
                        ($parcel$interopDefault($as8nN$lodash)).forEach(run.data, (val, key)=>{
                    ...
                    ($parcel$interopDefault($as8nN$lodash)).forEach(aggregatedRun.meta, (metaList, metaName)=>{
                        const uniqVals = ($parcel$interopDefault($as8nN$lodash)).uniq(metaList);
                        aggregatedRun.meta[metaName] = uniqVals.length == 1 ? uniqVals[0] : '--';
                    });
                    // consolidate the data values into the type of aggregate
                    ($parcel$interopDefault($as8nN$lodash)).forEach(aggregatedRun.data, (dataList, dataName)=>{
                        switch(this.selectedAggregateBy){
                            ...
                            case 'max':
                                aggregatedRun.data[dataName] = {
                                    value: ($parcel$interopDefault($as8nN$lodash)).max(dataList)
                                };
                                break;
                            case 'min':
                                aggregatedRun.data[dataName] = {
                                    value: ($parcel$interopDefault($as8nN$lodash)).min(dataList)
                                };
                                break;
                        }
                    });
    ...
    methods: {
        ...
        togglePathVisible (pathToToggle) {
            this.disabledFuncPaths = ($parcel$interopDefault($as8nN$lodash)).xor(this.disabledFuncPaths, [
                pathToToggle
            ]);
        },
        difference: ($parcel$interopDefault($as8nN$lodash)).difference,
...
var $15uoz = parcelRequire("15uoz");
var $5e6117824f544160$export$2e2bcd8739ae039 = ($parcel$interopDefault($as8nN$vue)).extend({
    data () {
        return {
            width: '100px',
            height: '100px'
    ...
    methods: {
        ...
        colorHash: $15uoz.colorHash,
        range: ($parcel$interopDefault($as8nN$lodash)).range
    },
    computed: {
        yticks () {
            const yticks = $as8nN$d3array.ticks(0, this.maxYval, 10);
            return $as8nN$d3array.zip(yticks.map(this.y), yticks);
        },
        seriesList () {
            const stackFunc = $as8nN$d3shape.stack().keys(this.displayedChildrenPaths);
            const viewData = this.runs.map((run)=>{
                return ($parcel$interopDefault($as8nN$lodash)).fromPairs(($parcel$interopDefault($as8nN$lodash)).map(this.displayedChildrenPaths, (path)=>[
                        path,
                        run.data[path].value
                    ]
                ));
        ...
        displayedXTitles () {
            return this.runs.map((run)=>{
                ...
                if ([
                    'launchdate',
                    'launchday'
                ].includes(this.selectedXAxisMetric)) return $as8nN$d3timeformat.timeFormat("%Y-%b-%d %H:%M")(new Date(parseInt(title + '000')));
                return title;
            });
        },
        numberOfTicks () {
        ...
        maxYval () {
            return $as8nN$d3array.max(this.seriesList, (d1)=>$as8nN$d3array.max(d1, (d)=>d[1]
                )
            );
        },
        y () {
            const scale = this.selectedScaleType == 'linear' ? $as8nN$d3scale.scaleLinear()// domain is [0, max value of seriesList]
            .domain([
                0,
                this.maxYval
            ]).range([
                this.height,
                0
            ]) : $as8nN$d3scale.scaleLog()// domain is [0, max value of seriesList]
            .domain([
                0.00001,
                this.maxYval
            ]).range([
        ...
        x () {
            const scale = $as8nN$d3scale.scaleLinear().domain([
                0,
                this.runs.length - 1
            ]).range([
                0,
        ...
        areaFunc () {
            return $as8nN$d3shape.area().x((d, i)=>this.x(i)
            ).y0((d)=>this.y(d[0])
            ).y1((d)=>this.y(d[1])
            );
        }
...
function $0cadd5f46329fa3f$export$cba17406db42b32d($0cadd5f46329fa3f$export$ab57e13899b83787, paths) {
    return ($parcel$interopDefault($as8nN$lodash)).filter(paths, (path)=>path.startsWith($0cadd5f46329fa3f$export$ab57e13899b83787) && path.split('/').length - $0cadd5f46329fa3f$export$ab57e13899b83787.split('/').length == 1
    );
}
...
function $0cadd5f46329fa3f$export$a2816e00bb6ecf08(text, alpha) {
    ...
    const hash = ($parcel$interopDefault($as8nN$md5))(reverseString);
...
const $9a7d627f7ddbf1bb$var$_withScopeId = (n)=>($as8nN$vue.pushScopeId("data-v-7f9af9"), n = n(), $as8nN$vue.popScopeId(), n)
;
...
const $9a7d627f7ddbf1bb$var$_hoisted_11 = /*#__PURE__*/ $9a7d627f7ddbf1bb$var$_withScopeId(()=>/*#__PURE__*/ $as8nN$vue.createElementVNode("line", {
        x1: 75,
        y1: 240,
        x2: 75,
        y2: 0,
...
function $9a7d627f7ddbf1bb$export$b3890eb0ae9dca99(_ctx, _cache) {
    return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", $9a7d627f7ddbf1bb$var$_hoisted_1, [
        $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_2, [
            $as8nN$vue.createElementVNode("div", null, $as8nN$vue.toDisplayString(_ctx.selectedGroupBy ? _ctx.selectedGroupBy + ': ' + _ctx.groupName : ''), 1 /* TEXT */ )
        ]),
        $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_3, [
            $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_4, [
                $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_5, [
                    $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_6, $as8nN$vue.toDisplayString(_ctx.selectedYAxisMeta), 1 /* TEXT */ )
                ]),
                $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_7, [
                    ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("svg", $9a7d627f7ddbf1bb$var$_hoisted_8, [
                        ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.yticks, (ytick)=>{
                            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, [
                                $as8nN$vue.createElementVNode("line", {
                                    x1: "70",
                                    x2: "75",
                                    y1: ytick[0],
                                    y2: ytick[0],
                                ...
                                $as8nN$vue.createElementVNode("text", {
                                    "text-anchor": "end",
                                    x: 65,
                                    y: ytick[0] + 4
                                }, $as8nN$vue.toDisplayString(ytick[1]), 9 /* TEXT, PROPS */ , $9a7d627f7ddbf1bb$var$_hoisted_10)
                            ], 64 /* STABLE_FRAGMENT */ );
            ...
            $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_12, [
                ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("svg", {
                    ...
                    $as8nN$vue.createElementVNode("rect", {
                        width: "100%",
                        height: "100%",
                        fill: "white",
                    ...
                    $as8nN$vue.createElementVNode("g", $9a7d627f7ddbf1bb$var$_hoisted_13, [
                        ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.seriesList, (series)=>{
                            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("path", {
                    ...
                    _ctx.hoverX && _ctx.hoverX.groupName == _ctx.groupName ? ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("line", {
                        key: 0,
                        ...
                        stroke: "black",
                        pointerEvents: "none"
                    }, null, 8 /* PROPS */ , $9a7d627f7ddbf1bb$var$_hoisted_15)) : $as8nN$vue.createCommentVNode("v-if", true)
                ], 32 /* HYDRATE_EVENTS */ ))
            ], 512 /* NEED_PATCH */ )
        ]),
        $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_16, [
            _ctx.i % Math.floor(_ctx.runs.length / _ctx.numberOfTicks) == 0 ? ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, {
                key: 0
            }, $as8nN$vue.renderList(_ctx.displayedXTitles, (xTitle, i)=>{
                return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", {
                    class: "xaxis-ticks",
                    style: $as8nN$vue.normalizeStyle({
                        position: 'absolute',
                    ...
                    $as8nN$vue.createElementVNode("span", {
                        style: $as8nN$vue.normalizeStyle({
                            position: 'absolute',
                            right: 0,
                            top: '10px',
                    ...
                    }, $as8nN$vue.toDisplayString(xTitle), 5 /* TEXT, STYLE */ )
                ], 4 /* STYLE */ );
            }), 256 /* UNKEYED_FRAGMENT */ )) : $as8nN$vue.createCommentVNode("v-if", true)
        ]),
        $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_17, [
            $as8nN$vue.createElementVNode("div", $9a7d627f7ddbf1bb$var$_hoisted_18, $as8nN$vue.toDisplayString(_ctx.selectedXAxisMetric), 1 /* TEXT */ )
        ])
    ]);
}
if (null) null.accept(()=>{
        ...
        addInclusive (selectedRunData) {
            ...
            function addInclusiveNode(nodeName) {
                ...
                if (!childrenFuncPaths.length) inclusiveData[nodeName] = {
                    ...
                    const childrenNodes = ($parcel$interopDefault($as8nN$lodash)).filter(inclusiveData, (value, key)=>childrenFuncPaths.includes(key)
                    );
                    const childrenSum = ($parcel$interopDefault($as8nN$lodash)).sumBy(childrenNodes, (node)=>node.inclusive
                    );
                    inclusiveData[nodeName] = {
                        inclusive: ($parcel$interopDefault($as8nN$lodash)).max([
                            myValue,
                            childrenSum
                        ]),
                        exclusive: myValue,
            ...
            addInclusiveNode(($parcel$interopDefault($as8nN$lodash)).min(funcPathKeys));
...
const $cd1a308e9290fd60$var$_withScopeId = (n)=>($as8nN$vue.pushScopeId("data-v-4af12e"), n = n(), $as8nN$vue.popScopeId(), n)
;
...
function $cd1a308e9290fd60$export$b3890eb0ae9dca99(_ctx, _cache) {
    const _component_FlamegraphNode = $as8nN$vue.resolveComponent("FlamegraphNode", true);
    return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", {
        class: "flamenode",
        style: $as8nN$vue.normalizeStyle({
        ...
        !_ctx.title.startsWith('--root') ? ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", {
            key: 0,
            class: "inclusive",
            style: $as8nN$vue.normalizeStyle({
                display: 'flex',
            ...
            $as8nN$vue.createElementVNode("div", {
                ...
                $as8nN$vue.createElementVNode("div", {
                    ...
                    title: _ctx.title
                }, $as8nN$vue.toDisplayString(_ctx.title), 13 /* TEXT, STYLE, PROPS */ , $cd1a308e9290fd60$var$_hoisted_1)
            ], 4 /* STYLE */ )
        ], 4 /* STYLE */ )) : $as8nN$vue.createCommentVNode("v-if", true),
        $as8nN$vue.createElementVNode("div", $cd1a308e9290fd60$var$_hoisted_2, [
            ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.childrenPaths(_ctx.funcPath, _ctx.allFuncPaths), (fp)=>{
                return $as8nN$vue.openBlock(), $as8nN$vue.createBlock(_component_FlamegraphNode, {
...
function $469bb275872bb446$export$b3890eb0ae9dca99(_ctx, _cache) {
    const _component_FlamegraphNode = $as8nN$vue.resolveComponent("FlamegraphNode");
    return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", $469bb275872bb446$var$_hoisted_1, [
        $as8nN$vue.createElementVNode("div", {
            class: "rootbutton",
        ...
        _ctx.funcPath != '--root path--' ? ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, {
            key: 0
        }, $as8nN$vue.renderList(_ctx.collapsedFuncPaths, (funcPath)=>{
            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", {
                ...
                $as8nN$vue.createElementVNode("div", $469bb275872bb446$var$_hoisted_3, $as8nN$vue.toDisplayString(_ctx.title(funcPath)), 1 /* TEXT */ )
            ], 8 /* PROPS */ , $469bb275872bb446$var$_hoisted_2);
        }), 256 /* UNKEYED_FRAGMENT */ )) : $as8nN$vue.createCommentVNode("v-if", true),
        $as8nN$vue.createVNode(_component_FlamegraphNode, {
...
const $01f753dff44a36fb$var$_withScopeId = (n)=>($as8nN$vue.pushScopeId("data-v-d63769"), n = n(), $as8nN$vue.popScopeId(), n)
;
...
const $01f753dff44a36fb$var$_hoisted_5 = /*#__PURE__*/ $01f753dff44a36fb$var$_withScopeId(()=>/*#__PURE__*/ $as8nN$vue.createElementVNode("label", {
        for: "xAxis-select",
        style: {
            "margin": "5px"
        }
...
const $01f753dff44a36fb$var$_hoisted_6 = /*#__PURE__*/ $01f753dff44a36fb$var$_withScopeId(()=>/*#__PURE__*/ $as8nN$vue.createElementVNode("label", {
...
const $01f753dff44a36fb$var$_hoisted_7 = /*#__PURE__*/ $01f753dff44a36fb$var$_withScopeId(()=>/*#__PURE__*/ $as8nN$vue.createElementVNode("label", {
        for: "yAxis-select",
        style: {
            "margin": "5px"
        }
...
const $01f753dff44a36fb$var$_hoisted_9 = /*#__PURE__*/ $01f753dff44a36fb$var$_withScopeId(()=>/*#__PURE__*/ $as8nN$vue.createElementVNode("label", {
        for: "groupBy-select",
        style: {
            "margin": "5px"
        }
...
function $01f753dff44a36fb$export$b3890eb0ae9dca99(_ctx, _cache) {
    const _component_view_chart = $as8nN$vue.resolveComponent("view-chart");
    const _component_FlameGraph = $as8nN$vue.resolveComponent("FlameGraph");
    return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", $01f753dff44a36fb$var$_hoisted_1, [
        $as8nN$vue.createElementVNode("div", {
        ...
        $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_2, [
            $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_3, [
                $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_4, [
                    $01f753dff44a36fb$var$_hoisted_5,
                    $as8nN$vue.withDirectives($as8nN$vue.createElementVNode("select", {
                        id: "xAxis-select",
                        "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event)=>_ctx.xAxis = $event
                        )
                    }, [
                        ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.xAxisList, (option)=>{
                            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("option", null, $as8nN$vue.toDisplayString(option), 1 /* TEXT */ );
                        }), 256 /* UNKEYED_FRAGMENT */ ))
                    ], 512 /* NEED_PATCH */ ), [
                        [
                            $as8nN$vue.vModelSelect,
                            _ctx.xAxis
                        ]
                    ]),
                    ...
                    $as8nN$vue.withDirectives($as8nN$vue.createElementVNode("select", {
                        ...
                        ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList([
                            '',
                            'sum',
                            'avg',
                            'min',
                            ...
                            return $as8nN$vue.createElementVNode("option", null, $as8nN$vue.toDisplayString(option), 1 /* TEXT */ );
                        }), 64 /* STABLE_FRAGMENT */ ))
                    ], 512 /* NEED_PATCH */ ), [
                        [
                            $as8nN$vue.vModelSelect,
                    ...
                    $as8nN$vue.createElementVNode("select", {
                        id: "yAxis-select",
                        value: _ctx.yAxis,
                        onInput: _cache[3] || (_cache[3] = ($event)=>_ctx.yAxisSelected($event.target.value)
                        )
                    }, [
                        ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.yAxisList, (option)=>{
                            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("option", null, $as8nN$vue.toDisplayString(option), 1 /* TEXT */ );
                        }), 256 /* UNKEYED_FRAGMENT */ ))
                    ...
                    $as8nN$vue.withDirectives($as8nN$vue.createElementVNode("select", {
                        ...
                        ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.groupByList, (option)=>{
                            return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("option", null, $as8nN$vue.toDisplayString(option), 1 /* TEXT */ );
                        }), 256 /* UNKEYED_FRAGMENT */ ))
                    ], 512 /* NEED_PATCH */ ), [
                        [
                ...
                }, "Set " + $as8nN$vue.toDisplayString(_ctx.selectedScaleType == 'linear' ? 'Log' : 'Linear'), 1 /* TEXT */ )
            ]),
            $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_10, [
                $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_11, [
                    ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.displayedChildrenPaths, (path)=>{
                        return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", {
                            class: "legend-item",
                            ...
                            }, $as8nN$vue.toDisplayString(_ctx.legendItem(path)), 5 /* TEXT, STYLE */ )
                        ], 8 /* PROPS */ , $01f753dff44a36fb$var$_hoisted_12);
                    }), 256 /* UNKEYED_FRAGMENT */ ))
                ])
            ])
        ]),
        $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_13, [
            ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.groupedAndAggregated, (runs, groupName)=>{
                return $as8nN$vue.openBlock(), $as8nN$vue.createBlock(_component_view_chart, {
                    groupName: groupName,
                    hoverX: _ctx.hoverX,
                    runs: runs,
        ...
        _ctx.selectedRun ? ($as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", $01f753dff44a36fb$var$_hoisted_14, [
            $as8nN$vue.createVNode(_component_FlameGraph, {
            ...
            $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_15, [
                ($as8nN$vue.openBlock(true), $as8nN$vue.createElementBlock($as8nN$vue.Fragment, null, $as8nN$vue.renderList(_ctx.selectedRun.meta, (metaVal, metaName)=>{
                    return $as8nN$vue.openBlock(), $as8nN$vue.createElementBlock("div", $01f753dff44a36fb$var$_hoisted_16, [
                        $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_17, $as8nN$vue.toDisplayString(metaName), 1 /* TEXT */ ),
                        $as8nN$vue.createElementVNode("div", $01f753dff44a36fb$var$_hoisted_18, ": " + $as8nN$vue.toDisplayString(metaVal), 1 /* TEXT */ )
                    ]);
                }), 256 /* UNKEYED_FRAGMENT */ ))
            ])
        ])) : $as8nN$vue.createCommentVNode("v-if", true)
    ]);
}
if (null) null.accept(()=>{
...
class $4fa36e821943b400$export$614db49f3febe941 {
    constructor(selector){
        this.app = new ($parcel$interopDefault($as8nN$vue))({
            el: selector,
            render: (h)=>h('App')
            ,
            components: {
    ...
    async getData(host, command, dataSetKey) {
        ...
        if (bust_cache) {
            ...
            cachedDataGet = await ($parcel$interopDefault($as8nN$localforage)).getItem(dataSetKey) || {
                Runs: {},
                RunDataMeta: {},
                RunGlobalMeta: {},
                RunSetMeta: {}
    ...
    async afterCachedDataGet(cachedDataGet, bust_cache, mtime, dataSetKey, host, command) {
        ...
        ($parcel$interopDefault($as8nN$lodash)).forEach(cachedData.Runs, (run, filename)=>{
        ...
        await ($parcel$interopDefault($as8nN$localforage)).setItem(dataSetKey, lr0);
        ...
        const visibleCharts = await ($parcel$interopDefault($as8nN$localforage)).getItem("show:" + dataSetKey) || $4fa36e821943b400$var$defaultVisibleCharts;
        ...
        summary.layout.scatterplots = await ($parcel$interopDefault($as8nN$localforage)).getItem('scatterplots:' + this.dataSetKey) || [];
        return summary;
    }
    async addScatterplot(options) {
        const key = "scatterplots:" + this.dataSetKey;
        const scatterplots = await ($parcel$interopDefault($as8nN$localforage)).getItem(key) || [];
        scatterplots.push(options);
        ($parcel$interopDefault($as8nN$localforage)).setItem(key, scatterplots);
    }
    async setChartVisible(globalName, visible = true) {
        const showGlobals = new Set(await ($parcel$interopDefault($as8nN$localforage)).getItem("show:" + this.dataSetKey) || $4fa36e821943b400$var$defaultVisibleCharts);
        if (visible) showGlobals.add(globalName);
        else showGlobals.delete(globalName);
        ($parcel$interopDefault($as8nN$localforage)).setItem("show:" + this.dataSetKey, Array.from(showGlobals));
    }
    compare(filenames) {
